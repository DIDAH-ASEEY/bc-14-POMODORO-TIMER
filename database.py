...
def set_shortBreak_db():
    ...
    conn = sqlite3.connect("pomodoro_timer.db")
    c = conn.cursor()
    c.execute("UPDATE pomodoro SET SHORT_BREAK = '%s' WHERE ID = '%s' " %
              (time, task_id))
    conn.commit()
    c.close()
...
def set_longBreak_db():
    ...
    conn = sqlite3.connect('pomodoro_timer.db')
    c = conn.cursor()
    c.execute("UPDATE pomodoro SET LONG_BREAK = '%s' WHERE ID ='%s' " %
              (time, task_id))
    conn.commit()
    c.close()
...
def set_sound_db():
    ...
    conn = sqlite3.connect("pomodoro_timer.db")
    c = conn.cursor()
    c.execute("UPDATE pomodoro SET SOUND='%s' WHERE ID='%s' " %
              (sound, task_id))
    conn.commit()
    c.close()
